...
/* ===== JsXray 适配导出（以下为新增，上方为 Heimdallr 原始规则库） ===== */
if (typeof self !== 'undefined') {
    self.printerData = printerData;
    self.HPrinter = HPrinter;
    self.HBlockingDomainRules = HBlockingDomainRules;
    self.HStats = {
        total: printerData.length,
        fingerprint: printerData.filter(r => r.type === 1).length,
        keyword: printerData.filter(r => r.type === 2).length,
        blockUrl: printerData.filter(r => r.type === 3).length,
        blockJsonp: printerData.filter(r => r.type === 4).length,
        alert: printerData.filter(r => r.type === 5).length,
        blockingRules: HBlockingDomainRules.length
    };
}
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { printerData, HPrinter, HBlockingDomainRules };
}
